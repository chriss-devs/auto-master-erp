...
### BL-024 · Asistente: chatbot sobre la BD con herramientas curadas (no RAG, no text-to-SQL)
Spec: `docs/superpowers/specs/2026-07-11-asistente-chatbot-design.md`; plan: `docs/superpowers/plans/2026-07-11-asistente-chatbot.md`. Tool calling con DeepSeek `deepseek-chat` (fetch nativo, sin SDKs) y 8 herramientas de SOLO lectura que reusan las queries existentes; RBAC por herramienta (se filtran antes de presentarlas al modelo + re-chequeo al ejecutar), dinero acotado a la sucursal activa y stock con visibilidad cruzada (D-030). Servidor stateless: historial en el cliente (`sessionStorage`), máx. 5 rondas de tools, historial sanitizado (solo `user`/`assistant`; el system prompt y los tool results los pone siempre el servidor). `buscar_cliente` solo expone compras si el usuario tiene `ventas:ver` y acotadas a sus sucursales (dos fugas de autorización detectadas por revisión de seguridad automática del commit y corregidas con tests). Widget flotante con render seguro: solo `**negritas**` y enlaces internos `[t](/ruta)` (rechaza `//` protocol-relative, `/\` y externos). Descartado text-to-SQL por imposibilidad práctica de garantizar RBAC en SQL generado. Env nuevas del api: `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL`.
...
- 2026-07-08 · Feedback del dueño aplicado: (a) fix placeholder de Next en `/`; (b) sidebar fijo a viewport con pie de sesión siempre visible; (c) dashboard rediseñado accionable (utilidad bruta y margen del día, comparativo vs ayer, cobros por método, efectivo en caja + último cierre, por-cobrar con antigüedad, inventario valorizado); (d) descuentos por % o monto en el POS (RN-160 intacto: el umbral se valida en el servidor sobre el monto); (e) precios especiales con ayuda de % (se persiste el precio, D-024); (f) alta rápida de categoría (con tipo/padre, RN-020) y de marca desde el formulario de producto. E2E 16/16 OK tras el redeploy.
...
- 2026-07-11 · **Asistente (BL-024)** construido en rama `feat/asistente-chatbot` (SDD, 7 tareas): `common/fechas` compartido, cliente DeepSeek (fetch), 8 herramientas RBAC de solo lectura, servicio con bucle de tool calls, endpoint `POST /asistente/chat` + integración con DeepSeek falso, widget flotante. Unit 40/40 + typecheck/lint/build verdes en api y web. Revisión de seguridad automática detectó y se corrigieron 2 fugas en `buscar_cliente` (compras cruzaban sucursal y no exigían `ventas:ver`). **Pendiente:** correr la integración con BD real, configurar `DEEPSEEK_API_KEY`/`DEEPSEEK_MODEL` en Vercel, desplegar, smoke en producción y **rotar la clave DeepSeek compartida por chat**.
